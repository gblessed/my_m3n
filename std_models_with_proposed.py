...
class Baseline_cbr_mb(tf.keras.Model):
    ...
    def __init__(self, override_mean_std_scores=None, name=None):
        ...
        self.iterations = 12 
        self.path_state_dim = 24 *2
        self.link_state_dim = 24 
    ...
    def call(self, inputs):
        ...
        device_state = tf.math.reduce_sum(tf.gather(link_state, device_to_link, name="DeviceToPath"), axis=1)
        ...
        for _  in range(self.iterations):
            ...
            device_gather =  tf.gather(device_state, device_to_path, name="DeviceToPath")
    

            link_gather = tf.concat([link_gather, device_gather], axis=2)
            ...
            # device_gather_score = tf.math.reduce_sum(device_path_gather, axis=1)
            path_gather_score = self.projection(path_gather_score)

            link_state, _ = self.link_update(path_gather_score, states=link_state)
            device_gather_score =  tf.math.reduce_sum(tf.gather(link_state, device_to_link, name="DeviceToPath"), axis=1)

            device_state, _ = self.device_update(device_gather_score, states=device_state)
