...
class Baseline_cbr_mb(tf.keras.Model):
    ...
    def call(self, inputs):
        ...
        lesser_indices = tf.expand_dims(inputs["lesser_indices"], 1)
        greater_indices = tf.expand_dims(inputs["greater_indices"], 1)
        ...
        # load_capacities = inputs["load_capacities"]
        ...
        queue_delay = self.readout_path(path_state)
        ...
        queue_delay = tf.gather(queue_delay,greater_indices)
         
  
        # updates = tf.expand_dims(tf.zeros(len(lesser_indices), dtype=queue_delay.dtype),1)



        # queue_delay = tf.tensor_scatter_nd_update(queue_delay, lesser_indices, updates)
