...
class Baseline_cbr_mb(tf.keras.Model):
    ...
    def __init__(self, override_mean_std_scores=None, name=None):
        ...
        self.parameter_vector = self.add_weight(
            shape=(self.path_state_dim * 2,),
            trainable=True,
        ...
        # Use tf.Variable to create a learnable parameter vector
        # self.parameter_vector = tf.Variable(initial_value, trainable=True, name='parameter_vector')
    ...
    def call(self, inputs):
        ...
        for _ in range(self.iterations):
            ...
            ) ## [No_linksx flows usedxhidden dim]

            # Expand dimensions to allow broadcasting
            tensor_expanded_1 = tf.expand_dims(path_gather, axis=2)  # Shape: [5, 1, 16]


            tensor_expanded_2 = tf.expand_dims(path_gather, axis=1)  # Shape: [1, 5, 16]

            # Broadcast and concatenate along the last axis
            pairwise_concat = tf.concat([tensor_expanded_1 + tf.zeros_like(tensor_expanded_2),
                                        tensor_expanded_2 + tf.zeros_like(tensor_expanded_1)], axis=-1)
          
            attention_scores  = tf.ragged.map_flat_values(self.compute_dot_product, pairwise_concat)
         

            attention_scores = tf.nn.softmax  (tf.nn.leaky_relu(attention_scores), axis = 2)
            weighted_score = tf.matmul(attention_scores, path_gather)
            ...
            # print("normalized_score", normalized_score[0][0].shape, tf.math.reduce_sum(normalized_score[0][0]))
        ...
        # print("delays->", queue_delay[:5])
        

        return queue_delay
    def compute_dot_product(self, values):
        # Reshape values to ensure the last dimension is 32
        reshaped_values = tf.reshape(values, (-1, 32))
        dot_product = tf.reduce_sum(reshaped_values * self.parameter_vector, axis=-1)
        return tf.reshape(dot_product, tf.shape(values)[:-1])
