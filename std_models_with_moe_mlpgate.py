...
class Baseline_cbr_MoE(tf.keras.Model):
    ...
    def __init__(self, override_mean_std_scores=None, name=None):
        ...
        self.expert_gate = std_models_jitter_binary.Baseline_cbr_mb(name = "expertgate")
        # self.expert4 = Baseline_cbr_mb(name = "expert4")
        self.loss_tracker = tf.keras.metrics.Mean(name="loss")
        self.loss_tracker2 = tf.keras.metrics.Mean(name="loss2")
    ...
    def call(self, inputs):
        ...
        self.expert_gate.mean_std_scores = self.mean_std_scores
        route = self.expert_gate(inputs)

        mask1 = binary_activation(route)
        mask2 = binary_activation(1.0 - route)
        ...
        # print("route", route.shape, "mask", tf.cast(mask1, tf.float32).shape, )
        ...
        return tf.squeeze(delays,1), route
    ...
    def train_step(self, data):
        ...
        # cond = tf.less_equal(delay, tf.ones(tf.shape(delay))*1.0)
        delay_binary = tf.expand_dims(binary_activation2(delay),1)
        with tf.GradientTape() as tape1, tf.GradientTape() as tape2:
        # with tf.GradientTape() as tape1:

            delay_pred, binary = self(x, training=True)  # Forward pass
            # Compute our own lossMeanSquaredLogarithmicError
            # jitter_pred = delay_pred
            # pkts_pred  = delay_pred
            # mean_squared_logarithmic_error
            # print("delay",delay.shape, "delay_pred", delay_pred.shape)
            loss = tf.keras.losses.log_cosh(delay, delay_pred)
            loss_binary =  tf.keras.losses.binary_crossentropy(delay_binary,binary )
            # print(delay_binary[:10],binary[:10], loss_binary)
        ...
        trainable_vars = self.expert_gate.trainable_variables
        # # print("lennnn ", len(trainable_vars))
        gradients = tape2.gradient(loss_binary, trainable_vars)
        
        # # Update weights
        self.optimizer[1].apply_gradients(zip(gradients, trainable_vars))
        # del tape
        # Compute our own metrics
        self.loss_tracker.update_state(loss)
        self.loss_tracker2.update_state(loss_binary)
        ...
        return {"loss": self.loss_tracker.result(), "loss_class": self.loss_tracker2.result(), }
    ...
    def metrics(self):
        ...
        return [self.loss_tracker, self.loss_tracker2 , self.delay_metric, ]
    ...
    def test_step(self, data):
        ...
        x, delay, jitter, pktsdropped  = data
        # pktsdropped = pktsdropped/tf.squeeze(x["flow_traffic"], axis=1)
        performance_metrics = [delay,]
        # Compute predictions
        delay_pred, binary = self(x, training=False)
...
def binary_activation2(x):
    ...
    cond = tf.less_equal(x, tf.ones(tf.shape(x))*1.0)
    out = tf.where(cond, tf.zeros(tf.shape(x)), tf.ones(tf.shape(x)))

    return out

    # cond = tf.less_equal(delay, tf.ones(tf.shape(delay))*1.0)
    # delay_binary = tf.where(cond, tf.zeros(tf.shape(delay)), tf.ones(tf.shape(delay)))
