...
usage
python3 /home/ec2-user/ITU-ML5G-PS-007-GNN-m0b1us/std_train_multi.py -ds CBR+MB --ckpt-path moblus_multi 
...
wandb.login(key="fdf71079a5df5f6f31921c1d7fe5e0f2c5287cb5")
wandb.init(project='gnn', name='multi_metric')
...
def train_and_evaluate(
    ...
    for feats, delay, jitter, packets_dropped in ds_train:
        all_delays.extend(delay.numpy().tolist())
        all_jitter.extend(jitter.numpy().tolist())
        all_pkts.extend(packets_dropped.numpy().tolist())


    metrics_csv= pd.DataFrame({"jitter": all_jitter, "delay": all_delays, "packets":all_pkts})
    metrics_csv.to_csv("allmetrics.csv")
    # Assuming 'data' is your DataFrame and 'numerical_data' is the column with continuous numerical data
    # print("all_delays ",all_delays[:20])
        
    # plt.hist(all_delays)
    # sns.kdeplot(all_delays,)

    # print("delays statistics mean", np.array(all_delays).mean(), "std: ",np.array(all_delays).std(), "max: ", np.array(all_delays).max(), "min ",np.array(all_delays).min() )
    ...
    # print("all_pkts ",all_pkts[:20])

    # plt.hist(all_pkts)
    print("packets statistics mean", np.array(all_pkts).mean(), "std: ",np.array(all_pkts).std(), "max: ", np.array(all_pkts).max(), "min ",np.array(all_pkts).min() )

    # sns.kdeplot(all_pkts)
    
    counts, bins = np.histogram(all_pkts)
    # plt.stairs(counts, bins)
    plt.hist(bins[:-1], bins, weights=counts, range=(np.array(all_pkts).min(), np.array(all_pkts)))
    # plt.xlim((np.array(all_pkts).min()/2, np.array(all_pkts).max() *2))
    plt.title('Distribution of packets dropped')
    plt.xlabel('packets dropped')
    plt.ylabel('Frequency')
    plt.savefig("pkts_distribution.png")
    plt.show()

    # # print("jitter ",all_jitter[:20])
    # # plt.hist(all_jitter)
    # print("jitter statistics mean", np.array(all_jitter).mean(), "std: ",np.array(all_jitter).std(), "max: ", np.array(all_jitter).max(), "min ",np.array(all_jitter).min() )
    # sns.kdeplot(all_jitter)
    # plt.xlim((-np.array(all_jitter).min()/2, np.array(all_jitter).max() *2))
    ...
    model.compile(
        optimizer=optimizer,
        loss ={'PathReadout': loss, 
                'JitterReadout':loss,
                'pkt_lossReadout':loss, 
                 },
        metrics=metrics,
        run_eagerly=RUN_EAGERLY,
    )
    ...
    history = model.fit(
        ...
        validation_data=ds_val,
        epochs=epochs,
        callbacks=[cpkt_callback, tensorboard_callback, wandb.keras.WandbCallback()] + additional_callbacks,
        use_multiprocessing=True,
    )
...
if __name__ == "__main__":
    ...
    # Check the scenario
    if args.ds == "CBR+MB":
        ds_path = "/home/ec2-user/gnnet-ch23-dataset-cbr-mb/extracted_moblus_multi_cv"
        model = std_models_multi.Baseline_cbr_mb
    elif args.ds == "MB":
        ds_path = "/home/ec2-user/gnnet-ch23-dataset-cbr-mb/mb_extracted_cv"
        model = std_models_multi.Baseline_mb
    else:
