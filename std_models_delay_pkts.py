...
class Baseline_cbr_mb(tf.keras.Model):
    ...
    def call(self, inputs):
        ...
        return delay, pkts_dropped
    
    def train_step(self, data):
        x, delay, _, pktsdropped = data
        ...
        with tf.GradientTape() as tape:
            delay_pred, pkts_pred= self(x, training=True)  # Forward pass
        ...
        return {"loss": self.loss_tracker.result(), "train_delay_mape": self.delay_metric.result(),  "train_pkts_mape": self.pkts_metric.result(),}
    ...
    def metrics(self):
        ...
        return [self.loss_tracker, self.delay_metric, self.pkts_metric ]
    
    def compute_loss(self, delay, delay_pred, pktsdropped, pkts_pred, sample_weight= None):
    ...
    def test_step(self, data):
        ...
        x, delay, _, pktsdropped  = data
        pktsdropped = pktsdropped/tf.squeeze(x["flow_traffic"], axis=1)
        performance_metrics = [delay, pktsdropped]
        # Compute predictions
        delay_pred, pkts_pred = self(x, training=False)
        predictions = [delay_pred, pkts_pred ]

        # Updates the metrics tracking the loss
        self.compute_loss(delay, delay_pred, pktsdropped, pkts_pred)
