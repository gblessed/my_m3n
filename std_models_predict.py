...
class Baseline_cbr_mb(tf.keras.Model):
    ...
    def __init__(self, override_mean_std_scores=None, name=None):
        ...
        self.link_embedding = tf.keras.Sequential(
            [
                tf.keras.layers.Input(shape=3 + 6),
                tf.keras.layers.Dense(
    ...
    def call(self, inputs):
        ...
        # print("linkcapacity", link_capacity_and_node_type.shape,  link_capacity_and_node_type[:, 1].shape)
        ...
        for _  in range(self.iterations):
            ...
            # attention_coef = self.attention2(path_gather)
            # # attention_coef = self.a_T2(attention_coef)
            # normalized_score = K.softmax(attention_coef)
            # # print("normalized_score", normalized_score.shape, normalized_score[0].shape)
            # weighted_score = normalized_score * path_gather        
            # path_gather_score2 = tf.math.reduce_sum(weighted_score, axis=1)
            


            # attention_coef = self.attention3(path_gather)
            # # attention_coef = self.a_T3(attention_coef)
            # normalized_score = K.softmax(attention_coef)
            # # print("normalized_score", normalized_score.shape, normalized_score[0].shape)
            # weighted_score = normalized_score * path_gather        
            # path_gather_score3 = tf.math.reduce_sum(weighted_score, axis=1)



            # attention_coef = self.attention4(path_gather)
            # # attention_coef = self.a_T4(attention_coef)
            # normalized_score = K.softmax(attention_coef)
            # # print("normalized_score", normalized_score.shape, normalized_score[0].shape)
            # weighted_score = normalized_score * path_gather        
            # path_gather_score4 = tf.math.reduce_sum(weighted_score, axis=1)

            # path_gather_score = tf.concat([path_gather_score1, path_gather_score2, path_gather_score3, path_gather_score4], axis = 1)

            # path_gather_score = self.project_back(path_gather_score)
