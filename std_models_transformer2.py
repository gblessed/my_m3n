...
class Baseline_cbr_mb(tf.keras.Model):
    ...
    def call(self, inputs):
        ...
        # linkG= tf.gather(link_stat00, link_to_path, name="LinkToPath")
        ...
        # path_state_sequence = self.sample_encoder(Pnlink, training=False)  
        ...
        for _ in range(self.iterations):
            ...
            # )
            Pnlink = tf.concat([tf.expand_dims(path_state, axis=1), link_gather.to_tensor()], axis=1)
            ...
            path_state_sequence = tf.concat(
                [tf.expand_dims(path_state, 1), path_state_sequence], axis=1
            )
